...
describe('CIConfigResolver', () => {
    ...
    describe('with no config file', () => {
        ...
        it('should use default values', () => {
            const resolver = new ci_config_resolver_1.CIConfigResolver('/test');
...
`);
    ...
    describe('convenience functions', () => {
        ...
        it('should resolve config via convenience function', () => {
            const config = (0, ci_config_resolver_1.resolveCIConfig)('/test', {
                goVersion: '1.21',
            });
            expect(config.goVersion).toBe('1.21');
            expect(config.testEnabled).toBe(true);
        ...
        it('should check job enabled via convenience function', () => {
            expect((0, ci_config_resolver_1.isJobEnabled)('/test', 'test')).toBe(true);
            expect((0, ci_config_resolver_1.isJobEnabled)('/test', 'benchmark')).toBe(false);
        });
...
ci:
  test:
    enabled: false
`);
            expect((0, ci_config_resolver_1.isJobEnabled)('/test', 'test')).toBe(false);
            const config = (0, ci_config_resolver_1.resolveCIConfig)('/test', {
                testArgs: '-v -short ./...',
            });
            expect(config.testArgs).toBe('-v -short ./...');
...
//# sourceMappingURL=ci-config-resolver.test.js.map