...
export class UnifiedPRComment {
  ...
  async updateComment(results: CIResults) {
    ...
    await this.upsertComment(octokit, comment);
  }
  ...
  async setProcessingState() {
    ...
    const token = core.getInput('github-token') || process.env.GITHUB_TOKEN;
    if (!token) {
      console.log('No GitHub token found, skipping processing state update');
      return;
    }

    const octokit = github.getOctokit(token);
    const comment = this.formatProcessingComment();

    await this.upsertComment(octokit, comment);
  }

  private async upsertComment(octokit: ReturnType<typeof github.getOctokit>, body: string) {
...
*Generated by [go-actions](https://github.com/jrschumacher/go-actions)*`;
  ...
  private formatProcessingComment(): string {
    return `# Go Actions Report
...
export async function setProcessingState(options: PRCommentOptions = {}) {
  const commenter = new UnifiedPRComment(options);
...
export async function loadAllResults(): Promise<CIResults> {
