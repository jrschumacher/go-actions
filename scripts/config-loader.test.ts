...
`;
      ...
      expect(() => loader.loadConfig(testWorkingDir)).toThrow(ConfigLoaderError);
...
`;
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('another-unknown')
      );
...
`;
  ...
  describe('convenience function', () => {
    it('should work with loadConfig convenience function', () => {
      ...
      const config = loadConfig(testWorkingDir);
      expect(config.version).toBe(1);
      expect(config.ci?.test?.enabled).toBe(true);
    });
    ...
    it('should accept overrides in convenience function', () => {
      ...
      const config = loadConfig(testWorkingDir, { goVersion: '1.21' });
      expect(config.ci?.go?.version).toBe('1.21');
    });
  });
