# Wails v3 — go-actions Recommendation

One-shot CI/CD setup for Wails v3 desktop apps using go-actions. All CI runs on Linux (cheaper, faster). Only the release app build runs on macOS.
...
- **Linux build deps**: Wails uses GTK and WebKit on Linux. Without these, `go build` fails because CGO can't find the C headers.
- **`mkdir -p frontend/dist`**: Wails apps typically have `//go:embed all:frontend/dist` in `app.go`. In a clean CI checkout, this directory doesn't exist and Go compilation fails.
  ...
  release-app:
    ...
    steps:
      ...
      - name: Package and upload
        ...
        run: |
          # Adjust paths based on your wails3 build output
          cd bin
...
Wails apps can't use GoReleaser's `go build` — they need `wails3 build` for proper frontend embedding and native bundling. Skip the build and let GoReleaser handle the GitHub release only:
  ...
  exclusions:
    ...
    rules:
      - path: _test\.go
        linters:
          - errcheck
      # Wails v3 API uses British spelling (Minimise, Maximise, etc.)
      - path: main\.go
        linters:
          - misspell
